...
const Documentations = () => {
  ...
  const addOrEditDataHandler = (item?: IProductItem) => {
    if (item?.id) {
      navigate(`/documentation/add`, { state: { id: item.id } });
      return;
    }
    navigate('/documentation/add');
  };
  ...
  const { table, isTableData } = useTableDocumentations({
    ...
    editHandler: (item) => addOrEditDataHandler(item),
      ...
      <div className="p-5 w-full h-full">
        <CardEID className="rounded-xl">
          <div className="flex items-center justify-between gap-4">
            <Label title="Documentations Table" subTitle="Data table information" />
