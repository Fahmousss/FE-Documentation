        ...
        >
          ...
          <Form.Item
            ...
            shouldUpdate={(prev, curr) => {
              const prevVal = namePath.reduce((acc, key) => acc?.[key], prev)?.[field.name]?.name;
              const currVal = namePath.reduce((acc, key) => acc?.[key], curr)?.[field.name]?.name;
              return prevVal !== currVal;
            }}
          >
            {() => {
              const name = form.getFieldValue([...namePath, field.name, 'name']);
              return <span className="font-medium">{name || `Item ${i + 1}`}</span>;
            }}
          </Form.Item>
