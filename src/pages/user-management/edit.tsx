...
import SelectAction from '@/shared/select-action';
...
import { ICreateUserManagementBody, ITableUserManagement } from './utils/model';

const EditUserManagement = () => {
  const [form] = Form.useForm<ICreateUserManagementBody>();
  const navigate = useNavigate();
  const { state } = useLocation() as { state: { data: ITableUserManagement } };
  ...
  const { editUser } = useMutationUserManagement();

  useEffect(() => {
    if (state?.data) {
      const { username, role, list_menu } = state.data;

      form.setFieldsValue({
        username,
        role,
        child: list_menu.map((item) => ({
      ...
      // Initialize menuPermissions
      const initialPermissions: { [key: string]: string[] } = {};
      list_menu.forEach((item) => {
        initialPermissions[item.menu_name as string] = item.permissions;
      });
  ...
  const handleAddMenu = () => {
    ...
    form.setFieldValue('child', [...currentMenus, { menu_name: undefined, permissions: [] }]);
  };
  ...
  return (
    <>
      ...
      />
      <CardEID>
        <LabelEID title="Edit User Data" subTitle="Update user information and permissions" />
